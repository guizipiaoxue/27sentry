...
class CircleCaliDlio final : public rclcpp::Node {
 public:
  CircleCaliDlio() : Node("circle_cali_dlio") {
    ...
    max_position_jump_ = declare_parameter<double>("max_position_jump", 0.20);
        ...
        [this](geometry_msgs::msg::PoseStamped::ConstSharedPtr m) {
          ...
          if (!poses_.empty()) {
            ...
            if (dp > max_position_jump_ || da > max_angular_jump_ ||
                dp / dt > max_linear_speed_ || da / dt > max_angular_speed_) {
 ...
 private:
  void report() {
    ...
    std::vector<bool> inlier(poses_.size(), true);
    ...
    for (int iter = 0; iter < 5; ++iter) {
      ...
      size_t n = 0;
      for (size_t i = 0; i < poses_.size(); ++i) if (inlier[i]) {
      ...
      for (size_t i = 0; i < poses_.size(); ++i) if (inlier[i]) {
        const auto R = poses_[i].q.toRotationMatrix();
        if ((poses_[i].p - (c - R * c)).norm() > cutoff) { inlier[i] = false; changed = true; }
      }
      if (!changed) break;
    }
    size_t used = 0;
