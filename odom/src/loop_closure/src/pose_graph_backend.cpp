...
gtsam::Vector6 noiseSigmas(double rotation_sigma_deg,
...
class PoseGraphBackend final : public rclcpp::Node {
 public:
  PoseGraphBackend()
    ...
    map_voxel_size_ =
        declare_parameter<double>("backend.map_voxel_size", 0.20);
    map_publish_every_n_keyframes_ = declare_parameter<int>(
        "backend.map_publish_every_n_keyframes", 5);
    save_path_ = declare_parameter<std::string>(
        "backend.save_path", "optimized_map.pcd");
    save_leaf_size_ =
        declare_parameter<double>("backend.save_leaf_size", 0.10);
    ...
    prior_noise_ = gtsam::noiseModel::Diagonal::Sigmas(noiseSigmas(
        prior_rotation_sigma_deg_, prior_translation_sigma_));
    odometry_noise_ = gtsam::noiseModel::Diagonal::Sigmas(noiseSigmas(
        odom_rotation_sigma_deg_, odom_translation_sigma_));
    const auto loop_gaussian = gtsam::noiseModel::Diagonal::Sigmas(noiseSigmas(
    ...
    keyframe_sub_ = create_subscription<loop_closure::msg::Keyframe>(
        ...
        std::bind(&PoseGraphBackend::keyframeCallback, this,
                  std::placeholders::_1));
    loop_sub_ = create_subscription<loop_closure::msg::LoopConstraint>(
        "loop_constraint", reliable_qos,
        std::bind(&PoseGraphBackend::loopCallback, this,
                  std::placeholders::_1));
    ...
    save_map_service_ = create_service<std_srvs::srv::Trigger>(
        "save_map", std::bind(&PoseGraphBackend::saveMap, this,
 ...
 private:
  ...
  void validateParameters() const {
    ...
    if (map_frame_.empty() || odom_frame_.empty() || invalid_sigma ||
        ...
        map_voxel_size_ <= 0.0 || map_publish_every_n_keyframes_ <= 0 ||
        save_leaf_size_ <= 0.0 || save_path_.empty()) {
  ...
  void keyframeCallback(
    ...
    if (!keyframes_.empty() && message->id <= keyframes_.rbegin()->first) {
      RCLCPP_WARN(get_logger(), "ignoring out-of-order keyframe id %d",
                  message->id);
      return;
    }
    ...
    ++keyframes_since_map_publish_;
    if (keyframes_.size() == 1U ||
        keyframes_since_map_publish_ >= map_publish_every_n_keyframes_) {
      publishMap(map_voxel_size_);
      keyframes_since_map_publish_ = 0;
    }
  }
  ...
  Cloud::Ptr buildMap(float leaf_size) const {
    ...
    Cloud::Ptr filtered_map(new Cloud);
  ...
  void publishMap(double leaf_size) {
    const Cloud::Ptr map = buildMap(static_cast<float>(leaf_size));
  ...
  void saveMap(
    ...
    const Cloud::Ptr map = buildMap(static_cast<float>(save_leaf_size_));
  ...
  int map_publish_every_n_keyframes_;
  double save_leaf_size_;
  int keyframes_since_map_publish_ = 0;
